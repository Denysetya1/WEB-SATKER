{{-- <div class="w-[300px] px-2 py-4">
    <div class="flex flex-col items-center">
        @if ($record->profile_photo_path)
            <img alt="avatar" lazy="" class="bg-white -mt-16 fi-avatar object-cover object-center !w-24 !h-24 mb-3 rounded-full shadow-xl"
                src="{{asset('storage/'.$record->profile_photo_path)}}">
        @else
            <img alt="avatar" lazy="" class="bg-white -mt-16 fi-avatar object-cover object-center !w-24 !h-24 mb-3 rounded-full shadow-xl"
...
<div class="profile-card js-profile-card">
    <span class="pro
        @if ($record->pegawai->bidang_id == 1 OR $record->pegawai->bidang_id == 4)
            bg-warning-400
        @elseif ($record->pegawai->bidang_id == 2 OR $record->pegawai->bidang_id == 3)
            bg-danger-400
        @elseif ($record->pegawai->bidang_id == 5)
            bg-success-400
        @else
            bg-gray-400
        @endif
    ">{{$record->pegawai->bidang->deskripsi}}</span>
    <div class="profile-card__img">
        @if ($record->profile_photo_path)
            <img src="{{asset('storage/'.$record->profile_photo_path)}}" alt="profile card">
        @else
            <img src="{{asset('images/avatar_default.png')}}" alt="profile card">
        @endif
    </div>

    <div class="profile-card__cnt js-profile-cnt">
        <div class="profile-card__name">{{$record->name}}</div>
        <div class="profile-card__txt">{{$record->pegawai->no_nip}} | {{$record->pegawai->no_nrp}}</div>
        <div class="profile-card__txt">{{$record->email}}</div>
        <div class="profile-card-loc">
            <span class="profile-card-loc__txt">
                @if ($record->pegawai->jabatan)
                    {{$record->pegawai->jabatan}}
                @else
                    Jabatan Belum Di input
                @endif
            </span>
        </div>
        ...
        <div class="profile-card-inf">
            <div class="profile-card-inf__item">
                <div class="profile-card-inf__txt">Pangkat | Golongan</div>
                <div class="profile-card-loc__txt">
                    @if ($record->pegawai->pangkat_id)
                        {{$record->pegawai->pangkat->nama_gol}} | {{$record->pegawai->pangkat->golongan}}
                    @else
                        - | -
                    @endif
                </div>
            ...
            <div class="profile-card-inf__item">
                <div class="profile-card-inf__txt">Struktural</div>
                <div class="profile-card-loc__txt">
                    @if ($record->pegawai->struktural->desstruktur_id == 5)
                        Tidak Menjabat
                    @else
                        {{$record->pegawai->struktural->desstruktur->deskripsi}}
                    @endif
                </div>
            </div>
        </div>
